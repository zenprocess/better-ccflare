 ...
 *
 * Exported so the bootstrap helper can clamp the polling interval above this
 * value — a poll interval shorter than the per-request timeout would let a
 * new request start before the previous one finishes and stack in-flight
 * calls (review finding F6).
 */
export const MINIMAX_USAGE_REQUEST_TIMEOUT_MS = 5000;
